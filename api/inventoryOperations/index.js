...
const inventoryHttpTrigger = async function (req, context) {
    const headers = {
    ...
    try {
        ...
        if (response.ok) {
            await recordHistoryEvent(context, userIdForHistory, "INVENTORY_OPERATION_SUCCESS", operation, req.method, true, req.method === 'POST' ? await req.json() : null, response.status, data);
        }
        else {
            await recordHistoryEvent(context, userIdForHistory, "INVENTORY_OPERATION_FAILED", operation, req.method, false, req.method === 'POST' ? await req.json() : null, response.status, data);
        }
        return {
        ...
        await recordHistoryEvent(context, userIdForHistory, "INVENTORY_OPERATION_EXCEPTION", req.params.operation, req.method, false, req.method === 'POST' ? await req.json().catch(() => 'Error reading body') : null, 500, (error instanceof Error ? { errorMessage: error.message, errorStack: error.stack } : { errorMessage: "Unknown exception type" }));
        return {
            status: 500,
            headers,
            jsonBody: {
...
exports.default = inventoryHttpTrigger;
