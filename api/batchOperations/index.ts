...
interface HistoryEntity {
    ...
    timestamp: Date;
    actionType: string;   // e.g., "BATCH_OPERATION"
    ...
    try {
        ...
        if (response.ok) {
            await recordHistoryEvent(
                ...
                req.method, 
                true, 
                req.method === 'POST' ? await req.json() : null, // Await req.json() here if needed for history
                response.status,
                data
            );
        } else {
            await recordHistoryEvent(
                ...
                req.method, 
                false, 
                req.method === 'POST' ? await req.json() : null, // Await req.json() here
                response.status,
                data
            );
        }
        ...
        await recordHistoryEvent(
            context, 
            userIdForHistory, 
            "BATCH_OPERATION_EXCEPTION", 
            req.params.operation, // operation might not be defined if error is early
            req.method, 
            false, 
            req.method === 'POST' ? await req.json().catch(() => 'Error reading body') : null, 
            500, // Assuming 500 for caught exceptions
